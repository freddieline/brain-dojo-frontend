...
export const useMemorySequenceGameState = () => {
    ...
    const [selectableItems, setSelectable] = useState([]);
    const [currentIndex, setCurrentIndex] = useState(0);
    const [sequenceLength, setSequenceLength] = useState(4);
    const [gameState, setGameState] = useState(SequenceRecallGameState.PreGame);
    const [timeToMemorize, setTimeToMemorize] = useState(10);
    const setupGame = (sequenceLength) => {
        if (!sequenceLength) {
            const sequenceLengthTemp = 4;
    ...
    const nextLevel = () => {
        if (sequenceLength != 8) {
            const sequenceLengthTemp = sequenceLength + 1;
            setSequenceLength(sequenceLengthTemp);
        }
    };
    const start = () => {
    ...
    const updateGameState = () => {
        if (gameState === SequenceRecallGameState.Start)
            generateInitialCards();
        if (gameState === SequenceRecallGameState.Memorize)
            revealSequence();
    ...
    function revealSequence() {
        setSequenceItems((prevItems) => prevItems.map((item) => ({ ...item, show: true })));
        setTimeout(() => {
            setSequenceItems((prevItems) => prevItems.map((item) => ({ ...item, show: false })));
            setGameState(SequenceRecallGameState.Play);
        }, 1000 * timeToMemorize);
    }
    const handleSelection = (animal) => {
        setSelectable((prevItems) => prevItems.map((item) => ({
            ...item,
    ...
    function updateCardDisplay(index, options = {}) {
        setSequenceItems((prevItems) => prevItems.map((item, idx) => idx === index ? { ...item, ...options } : item));
    }
